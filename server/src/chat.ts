...
function registerChat(initializer: nkruntime.Initializer): void {
  initializer.registerRpc("social_chat_send", function (ctx, _logger, nk, payload) {
    const actor = authenticated(ctx);
  ...
  initializer.registerRpc("social_chat_history", function (ctx, _logger, nk, payload) {
  ...
  initializer.registerRtBefore<nkruntime.EnvelopeChannelJoin>("ChannelJoin", function (ctx, _logger, nk, envelope) {
  ...
  // Joining sockets subscribes to new messages. All writes and history reads
  // go through RPC so permissions are rechecked after unfriend/leave/kick.
